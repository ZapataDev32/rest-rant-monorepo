...
function PlaceDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`http://localhost:3030/places/${placeId}`);
  ...
  async function deletePlace() {
    await fetch(`http://localhost:3030/places/${place.placeId}`, {
      method: "DELETE",
    });
    history.push("/places");
  }
  ...
  async function createComment(commentAttributes) {
    const response = await fetch(
      `http://localhost:3030/places/${place.placeId}/comments`,
      {
        method: "POST",
        headers: {
